...
/* Step C:  Define Key Demographic Variables (employment status, groups, region, education, usual hours worked, etc.) */  
	...
	gen emp_full = empstat == 1 & hrswork2 >= 3 ;					/* This is a measure of whether working full time */
	gen emp_part = empstat == 1 & hrswork2 == 2; 					/* This is a measure of whether working part time - we assume working less than 15 hours a week is not working */
	...
	gen 		home_adj = . ; 										/* "home_adj" will be the variable we use for home sector counts - it counts part time workers as being 0.5 in home sector */ 
	...
	gen emp_full_lastyear = wkswork2 >= 5 & (incwage+incbusfm) >= 1000 * 29.61/207.16 ;   /*  This is to condition our earnings variables - it restricts sample to those working at least 48 weeks in prior year */
...
/*  Step D:  Create Harmonized Wage/Earnings Data */ ; 
	...
	replace incwage_full = incwage+incbusfm if emp_full_lastyear == 1 & emp_full_adj > 0;  	/*  Restrict this measure to only those who worked full time last year */             

	gen wks_last_year = . ;
	replace wks_last_year = 49 if wkswork2 == 5 ;
	replace wks_last_year = 51 if wkswork2 == 6 ;

	gen     uhrswork = 22 if hrswork2 == 2 ; 
	replace uhrswork = 32 if hrswork2 == 3 ; 
	replace uhrswork = 37 if hrswork2 == 4 ; 
	replace uhrswork = 40 if hrswork2 == 5 ; 
	replace uhrswork = 45 if hrswork2 == 6 ; 
	replace uhrswork = 55 if hrswork2 == 7 ; 
	replace uhrswork = 60 if hrswork2 == 8 ; 
	
	gen wage = incwage_full/(uhrswork * wks_last_year) ;										/* We will use this measure to compute wage gaps between groups */ 
...
/*   Part F:  Save the extract as a new file */ ; 
	...
	#delimit ; 
	save 1960_extract_composite_main, replace ; 
