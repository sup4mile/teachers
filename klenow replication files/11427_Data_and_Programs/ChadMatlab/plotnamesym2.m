...
if exist('markercolor')~=1; markercolor=[0 0.4470 0.7410]; end;
...
%h=plot(x,y,'.','Color',markercolor,'MarkerFaceColor',markercolor,'MarkerSize',20);
h=plot(x,y,'o','Color','k','MarkerFaceColor',markercolor,'MarkerSize',6,'LineWidth',.5);
N=length(x);
...
for i=1:N;
  ...
  if (~skipname | namethese(i)==1) & ~isnan(namethese(i));
...
if addols>0;
  ...
  [xmin,imin]=min(x);
  [xmax,imax]=max(x);
  plot([xmin xmax],yfit([imin imax]),'k-','LineWidth',1);
