...
# all labor market discrimination against women is eliminated.
# This is implemented by setting τ_w_opt = 0 and λf = 1, so that τ_w[:, 1] = 0 for all occupations.
# The parameter κ is taken as calibrated from each year respectively.
# The results illustrate what would have happened if women faced no workplace barriers.

# Key changes relative to the main solution (frechet.jl):
# (1) Set τ_w_opt = 0 and λf = 1 for women in all years (no barriers).
...
# (5) Recompute occupation shares and marginals for women using zero barriers.
...
println("  COUNTERFACTUAL 2: year = ", year, " with ALL barriers eliminated for women")
...
# Change: Eliminate ALL barriers for women by setting τ_w_opt = 0 and λf = 1
# With λf = 1 and τ_w_opt = 0: τ_w[:, 1] = ones(n_O-1) .- 1 * (ones(n_O-1) .- 0) = 0
global λf_counter
λf_counter = 1.0
τ_w_opt[:, 1] .= 0.0
τ_w[:, 1] = ones(n_O - 1) .- λf_counter * (ones(n_O - 1) .- τ_w_opt[:, 1])
println("Eliminating all barriers for women: λf = ", λf_counter, ", τ_w_opt = 0")
println("Resulting τ_w[:, 1] = ", τ_w[:, 1])
...
# Recompute occupation shares and marginals using zero barriers for women, unchanged for men
...
println("Counterfactual (no barriers) share of women in non-teaching occupations:")
...
println("Found fixed point for human capital in teaching (counterfactual 2 - no barriers)!")
...
println("aggA (counterfactual 2 - no barriers) = ", round.(aggA, digits=6))
...
cd(string("./parameterization/", paramname))
fnameJLD_cf = string("counter_2_", fyear, ".jld")
save(fnameJLD_cf, "a_by_occ", a_by_occ, "τ_w", τ_w, "τ_w_opt", τ_w_opt, "τ_e", τ_e, "a_T_thresh", a_T_thresh, "t", t, "H_grid", H_grid, "H_O", H_O, "HH_fp", HH_fp, "α", α, "β", β, "η", η, "σ", σ, "μ", μ, "ϕ", ϕ, "γ", γ, "κ", κ, "theta", theta, "λf_counter", λf_counter, "λm", λm, "iHH", iHH, "a_grid", a_grid, "a_O_10p", a_O_10p, "a_O_90p", a_O_90p, "a_T_10p", a_T_10p, "a_T_90p", a_T_90p, "h_T", h_T, "f_T", f_T, "f_O", f_O, "mass_T", mass_T[iHH, :], "mass_O", mass_O[iHH, :, :], "aggA", aggA, "share_occ", share_occ, "ω_90_10", ω_90_10, "w_90_10", w_90_10)
cd("..")
cd("..")

println("____________")
println("COUNTERFACTUAL 2 RESULTS (year = ", year, ", ALL barriers eliminated for women)")
...
println("")
println("Counterfactual 2 for year ", year, " complete.")
println("================================================================")
println("")
