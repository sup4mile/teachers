# This Julia script computes counterfactual steady states for 1970, 1990, and 2010 where the extent of labor market discrimination against women is frozen in 1970. The parameter κ is fixed at the values from the benchmark calibration for each steady state. By construction, the counterfactual results in 1970 match the baseline calibration. The results for 1990 and 2010 illustrate what would have happened if women's educational and labor market opportunities had remained frozen in time.
...
# Load parameters and results from previous parameterization:
    cd("./parameterization/counterfactuals/counter_3")
    fyear = string(year)
...
# κ = 0.169 # scale parameter of teachers' wage profile
...
for iH in 1:n_H
    ...
    while convT > tolT && iterT < maxiterT
        for iG in 1:n_G
            ...
            # Compute tomorrow's aggregate human capital in teaching ('HH_T'):
            HH_T_0[iG,iH]= ( (1-t[iH,1])^η*η^η*(2*H/M)^σ)^(β/σ/(1-η))*f2[iH,iG]
            # Total earnings of teachers in each group:
            E_T[iH,iG]=f4[iH,iG]
            # Total output of teachers in each group:
            ...
            h_T_avg[iH,iG] = quadgk(aa -> spl_h_T(aa)*pdf(dist,aa)*spl_f_1_T[iH,iG](aa),lowbnd,upbnd)[1] / mass_T[iH,iG]
        end
...
fnameCSV = string("./results/counterfactuals/counter_3/moments",fyear,".csv")
