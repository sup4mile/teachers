...
# labor market discrimination against women is eliminated in all occupations except home production. 
# This captures an intensive vs extensive margin effect by serving as
# an in-between of counterfactual 2 (all barriers eliminated).
#
# Key changes relative to the main solution (frechet.jl):
# (1) Set τ_w[:, 1] = 0 for all non-teaching occupations except home production (index n_O-1).
#     For home production, keep the calibrated τ_w from the main model solution for each year.
...
println("  COUNTERFACTUAL 3: year = ", year, " with barriers eliminated EXCEPT home production")
...
# For non-HP occupations (indices 1 to n_O-2): set τ_w_opt = 0, effectively λf = 1, so τ_w = 0.
# For home production (index n_O-1): keep the calibrated barriers from the main model solution.
τ_w_opt[:, 1] .= 0.0
τ_w[:, 1] .= 0.0
# Restore home production barrier (last non-teaching occupation) to calibrated level:
τ_w_opt[n_O-1, 1] = τ_w_initial[n_O-1, 1]
τ_w[n_O-1, 1] = 1.0 - λf * (1.0 - τ_w_opt[n_O-1, 1])
println("Eliminating barriers for women in all occupations EXCEPT home production")
println("τ_w[:, 1] for non-HP occupations = ", τ_w[1:n_O-2, 1])
println("τ_w[n_O-1, 1] for home production = ", τ_w[n_O-1, 1], " (calibrated)")
...
# Recompute occupation shares and marginals using modified barriers for women (zero except HP), unchanged for men
...
println("Counterfactual (no barriers except HP) share of women in non-teaching occupations:")
...
println("Found fixed point for human capital in teaching (counterfactual 3 - no barriers except HP)!")
...
println("aggA (counterfactual 3 - no barriers except HP) = ", round.(aggA, digits=6))
...
cd(string("./parameterization/", paramname))
fnameJLD_cf = string("counter_3_", fyear, ".jld")
...
println("COUNTERFACTUAL 3 RESULTS (year = ", year, ", barriers eliminated except home production)")
...
println("")
println("Counterfactual 3 for year ", year, " complete.")
println("================================================================")
println("")
