\documentclass[onehalfspacing,12pt]{article}
...
The model is populated by a constant measure $M$ of heterogeneous individuals. Each individual is born with an exogenous $N$-dimensional vector of skills $\vec{a}$, where the $n^{\rm th}$ element characterizes her innate ability in occupation $n$. This vector of abilities is drawn from a known joint distribution $F_a(\vec{a})$.

Individual agents live for two periods. They attend school when they are children (``young'') and they work when they are adults (``old''). For simplicity, we abstract from retirement and instead assume that everyone exits the economy when they reach the end of adulthood.

Each cohort has measure $\tfrac{M}{2}$ and therefore the age distribution of the population is constant over time.

\subsection{Technologies}

Children have access to a teaching technology that produces occupation-specific human capital with four inputs: their own ability $a^n$, a time investment $s$, an investment in terms of the final good $e$, and the teacher's human capital $h^T$:
  \begin{align*}
\label{eq:h'}
h^{n'}(a) & = \left( h^T\right)^\beta a^\alpha s\left(a\right)^\phi e(a^n)^\eta {N(h^T,\widetilde{H}^T)}^{-\sigma},
\end{align*}

where $\widetilde{H}^T = \int_0^\infty \left( h^T\left( a \right) \right)^{\frac{\beta}{\sigma}} f^T(a) da$ is an aggregate measure of human capital across all current teachers and $N(h^T,\widetilde{H}^T)$ denotes the class size of a teacher with human capital $h^T$. The density $f^T(a)$ characterizes the ability distribution of the current cohort of teachers.

Without loss of generality, occupation 1 is \textit{teaching}. In this economy, teachers are distinct from other occupations in that their own human capital is an input into a production function that enables their students to accumulate human capital. In contrast, the human capital of workers in other occupations is used to produce units of a homogenous final (consumption) good. This kind of distinction is salient since we are assuming that the economy is populated by overlapping generations of workers who accumulate human capital when young, which then pays off when they are of working age (or ``old'').
\subsection{Abilities and Distortions}
...
There are two types of distortions: educational barriers labeled $\tau^e$ and labor market discrimination labeled $\tau^w$. These distortions are specific to groups of individuals and we assume that there are $G$ different groups and we will label them with superscripts $g \in \{1,\ldots,G\}$. The distortions evolve according to Markov chains with transition matrices $P_{\tau^{e'},\tau^e} = \mathbb{P}^{e,n,g} \left( \tau_{t+1}^e = \tau^{e'} | \tau_{t}^e = \tau^{e} \right)$ and $P_{\tau^{w'},\tau^w} = \mathbb{P}^{w,n,g}\left( \tau_{t+1}^w= \tau^{w'} | \tau_{t}^w = \tau^{w} \right)$, respectively, for each occupation $n$ and group $g$.

The timing convention is similar to \cite{Hsieh:2018}. In the first period, prospective workers -- students, that is -- make human capital investments. Since there is no uncertainty, students have perfect foresight and their investment decision will depend on their fully anticipated occupational choice in period 2. They retire at the end of their working period and are replaced by a new cohort of students with measure $\tfrac{M}{2}$. The environment is stationary in the sense that the members of each cohort draw their ability vectors from the same distribution $F_a$.

The worker's utility depends on her consumption when she is old and her time devoted to human capital investment when she is young:
\begin{equation}
\label{ }
...
\begin{align}
\label{}
C_{t+1} & =\max \left\{ (1-\tau^{\omega,g,n})\omega_{t+1}(h_{t+1}^n)-e_{t} \right\}_{n \in N}, \\ %, \textrm{ for } t \in \{c,c+1\}  \\
h_{t+1}^n & =g(h_{t}^T,a^n) s_t(a^{\hat{n}})^{\phi} e_t \left(a^{\hat{n}} \right)^{\eta}\big(N_t(h_t^T)\big)^{-\sigma}, \label{eq:h}\\
g(h_{t}^T,a^n) & =(h_{t}^T)^\beta \left( a^n \right) ^\alpha,
\end{align}

{\bf Need to add marginal tax rate $t$!}

$C_{t+1}$ denotes consumption of the homogeneous final good when old, $\hat{n} \in N$ indexes the workers occupational choice when young, $s_t(\hat{n})$ is the time allocated to human capital formation, and $e_t(\hat{n})$ are units of the final good invested in human capital formation when workers are still students. These investments are occupation-specific (CAN WE RELAX THIS ASSUMPTION?) and indexed by the superscript $n$.\footnote{Nothing prevents students from making investments in human capital in multiple occupations. Since there is no uncertainty in future payoffs they will, however, select only one in equilibrium.} The periodic time endowment is set to unity. $1-s_t$ is leisure time when young and labor is supplied inelastically when individuals reach working age. $\tau^{\omega,g,n}$ is a labor market distortion (or wedge) that is specific to workers in group $g$ in occupation $n$.

A worker's vector of occupation-specific abilities is drawn from a multivariate distribution with c.d.f. 

The joint distribution of $\vec{a}$ is a multivariate Fr\'echet with c.d.f.~$F(a_1,\ldots,a_N)$. The draws from this distribution are independent across workers.
%\begin{equation}
%\label{ }
...
The production technologies in occupations $2,\ldots,N$ are linear in the workers human capital. A worker with human capital $h$ in occupation $n$ produces
\begin{equation}
\label{eq:techO}
y^n = h^n A^n
\end{equation}
...
Since industry output is homogeneous, aggregate output is given by
\begin{equation}
\label{ }
Y = \sum_{n=2}^N \int \textrm{To Be Completed!}
\end{equation}
...
Follow-up surveys were then carried out one, five, and eleven years after the students' expected high school graduation. For instance, students who were in eleventh grade in 1960 were expected to graduate in 1961 and were contacted again in 1962, 1966, and 1972. Ninth graders were contacted in 1964, 1968, and 1975. A lack of federal funding stopped the project during the planning stage for a 17-year follow-up, which never took place.
