...
\begin{document}
	...
	Project TALENT 11-year follow up report information on 29-year-old individuals who attended at least one year of high school. We select equivalent samples from NLSY79 and NLSY97. Census and ACS samples include individuals who attend at least one year of high school and ``young`` age group (age 25-34). 
	
	We define a person working in the home production is out of labor force or works less than 15 hours per week. Individuals working at least 30 hours are classified as employed in one of 21 occupations. Those who work between 15 and 29 hours are classified as part-time workers. We split their sampling weight equally between the home production and the reported market occupation.
	...
	We assume that the idiosyncratic occupation-specific abilities are drawn from a Fr\'echet distribution and the relative innate talent levels are identical across all groups. Focusing on the one age group (``young``), we cannot differentiate between labor market barriers and human capital barriers. Therefore, we assume that educational barriers are zero. In addition, we assume that men face no labor market barriers, while female workers face labor market in all occupations, besides teaching. We normalize elasticities of human capital with respect to ability and class size to one. Table \ref{tab:assump} summarizes the assumptions and normalizations.
	
	To estimate $\theta$ and $\eta$ to infer non-teaching occupational productivities. To estimate $\theta$, the distributional assumptions imply that wages within an occupation for a given group follow a Fr\'echet distribution with the shape parameter $\theta(1-\eta)$. Using Census/ACS data, we estimate $\theta(1-\eta)$ to fit the distribution of the residuals from a cross-sectional regression of log hourly wages on 20 x 2 x 10 occupation-group-age dummies in each year. The resulting estimates for $\theta(1-\eta)$ is 1.324. The parameter $\eta$ denotes the elasticity of human capital with respect to goods investment and is equal to the fraction of output spent on human capital accumulation. Public plus private spending on education as a share of GDP in the United States is, on average, 6.6 percent over the years 1995, 2000, and 2010. Since the labor share in the United States in the same three years was 0.641, this implies an $\eta$ of 0.103. With $\theta(1-\eta)$=1.324 and $\eta$=0.103, we get $\theta$=1.476.
	
	To estimate $\gamma$, distributional assumptions imply that wages within teaching occupation for a given group follow a Fr\'echet distribution with the shape parameter $\theta(1-\gamma\eta)$. We estimate $\theta(1-\gamma\eta)$ to fit the distribution of the residuals from a cross-sectional regression of log hourly wages on 2 x 10 group-age dummies in each year. We get $\theta(1-\gamma\eta)$ equal to 1.350 across years. Then, $\theta$=1.476 and $\eta$=0.103 give us $\gamma$=0.830.
	
	We pick $\phi$ to match the Mincerian return from a cross-sectional regression of log income on years of schooling (with occupation x group dummies), which gives us $\phi$=2.745. Schooling of teachers relative to schooling of non-teaching workers is 1.23 which gives us $\mu$=0.714. Table \ref{tab:param} summarizes the parameters.
	
	Table \ref{tab:param_time} summarizes the parameters that vary over time. Occupational productivities could be recovered from the data on non-teaching occupational shares for men at each year. We normalize home production productivity to be one. Figure \ref{fig:prod} illustrates occupational productivities over time. We pick labor market barriers in non-teaching occupations to match female non-teaching occupational shares at each year. Figure \ref{fig:bar} presents labor market barriers over time. The parameter for the aggregate productivity in teaching $\kappa$ is determined by matching the fraction of men who become teachers. We get 0.2895,0.190, and 0.169 for 1970, 1990, and 2010, correspondingly. The aggregate labor market barrier for women in non-teaching occupations $\lambda_f$ is imputed from the fraction of female choosing teaching occupation. The estimates are 0.879, 1.034, and 0.602 for 1970, 1990, and 2010, correspondingly.
	...
	\begin{figure}
		\begin{center}
			...
			\caption{Distribution of Female Teaching Abilities}
			\label{fig:fem_ab}
		\end{center}
	\end{figure}
	...
	\begin{figure}
		\begin{center}
			...
			\caption{Distribution of Male Teaching Abilities}
			\label{fig:m_ab}
		\end{center}
	\end{figure}
	...
	\begin{figure}
		\begin{center}
			...
			\caption{Human Capital Investment}
			\label{fig:humcap}
		\end{center}
	\end{figure}
