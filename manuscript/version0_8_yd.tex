...
\begin{equation}
\label{ }
U_t = \max_{C_{t+1},s_t,e_t}\ln C_{t+1} + \mu\ln\left(1-s_t\right) \label{eq:util}
\end{equation}
where
\begin{align}
\label{}
C_{t+1} & =(1-t)(1-\tau^{\omega,g})\omega_{t+1}(h_{t+1})-(1+\tau^{e,g})e_{t} \label{eq:bc}\\ %, \textrm{ for } t \in \{c,c+1\}  \\
...
\begin{enumerate}
  \item Given an occupational choice and the occupational wage, each individual chooses consumption $C$, goods investment $e$, and time investment $s$ to maximize utility given by (\ref{eq:util}) subject to the constraints given by (\ref{eq:bc}).
  \item Each individual chooses the occupation that maximizes utility [COMPLETE!].
  \item The aggregate human capital follows the laws of motion given by (\ref{eq:lomT}) &and (\ref{eq:lomO}).
\end{enumerate}
\end{def}
...
\begin{align}
\label{eq:foc-e}
(1-\tau^e) e^t \left( \frac{\sigma}{\beta \eta}-1 \right) & = \left(1-s^T\right) \mu(1-t)(1-\tau^w) \frac{\partial \omega'}{\partial s^T} \\
\label{eq:foc-s}
(1-t)(1-\tau^w) \frac{\partial \omega'}{\partial e^T} & = 1-\tau^e,
\end{align}
where
\begin{align}
%\label{}
...
The F.O.C.s for prospective production workers are analogues of \eqref{eq:foc-e} and \eqref{eq:foc-s} with the simplification that the wage profile in production (as opposed to teaching) is linear in $h^O$. The amount of time and goods an individual spends on human capital are then

\begin{align}
%\label{eq:s_T}
s_{T,g} & = \frac{\mu \phi}{\mu \phi+\tfrac{\beta}{\sigma}-\eta} \\
%\label{eq:e_T}
e_{T,g} & = \tau_{T,g}\cdot\tfrac{\beta}{\sigma}\cdot\eta^{\frac{1}{1-\eta}}\cdot \left(\tfrac{2\widetilde{H}^T}{M}\right)^{\frac{\sigma}{1-\eta}} \nonumber\\
...
\begin{align}
%\label{eq:s_O}
s_{i,g} & = \frac{\mu \phi}{\mu \phi+1-\eta} \\
%\label{eq:e_O}
e_{i,g} & = \left( \tau_{i,g} \cdot \eta \cdot \left(\tfrac{2\widetilde{H}^T}{M}\right)^\sigma\cdot A'_i^\eta \cdot s_T^\phi \cdot a_i^\alpha \right)^{\frac{1}{1-\eta}}
\end{align}
where
\begin{align}
...
\begin{align}
%\label{eq:lomT}
\widetilde{H'}^{T} & = \sum_g \Bigg[ \left \tau_{T,g}^\eta \cdot \left(\tfrac{\beta}{\sigma}\right)^\eta  \cdot \eta^{\frac{\eta}{1-\eta}} \cdot \left(\tfrac{2\widetilde{H}^T}{M}\right)^{\frac{\sigma}{1-\eta}} \cdot s_T^\phi \nonumber\\
...
& \times \left(\int_0^\infty a^{\frac{\alpha}{\frac{\sigma}{\beta}-\eta}} f^{T,g}(a)da\right)^{\frac{\sigma}{\beta}-\eta} \Bigg]^{\alert{\frac{\beta}{\sigma}}}\\
...
\subsection{Occupational Wages in Decentralized Competitive Equiliribium (No Frictions)}
...
\begin{align}
%\label{eq:ss}
\widetilde{H}^{T} & = \Bigg\{\sum_g \Bigg[ \left \tau_{T,g}^\eta \cdot \left(\tfrac{\beta}{\sigma}\right)^\eta  \cdot \eta^{\frac{\eta}{1-\eta}} \cdot \left(\tfrac{2}{M}\right)^{\frac{\sigma}{1-\eta}} \cdot s_T^\phi \nonumber\\
& \times \left(\frac{\sum_i {\tau_{i,g}}^{\frac{\eta}{1-\eta}} \cdot A'_i^{\frac{1}{1-\eta}} \cdot s_i^{\frac{\phi}{1-\eta}} \cdot \int_0^\infty a^{\frac{\alpha}{1-\eta}} f^{i,g}(a)da }{\sum_i \int_0^\infty f^{i,g}(a)da}\right)^\eta \nonumber\\
& \times \left(\int_0^\infty a^{\frac{\alpha}{\frac{\sigma}{\beta}-\eta}} f^{T,g}(a)da\right)^{\frac{\sigma}{\beta}-\eta} \Bigg]^{\alert{\frac{\beta}{\sigma}}} \Bigg\}^{\frac{1-\eta}{1-\eta-\beta}}\\
{H}^{i} & = 
\end{align}
where
\begin{align}
...
Table \ref{tab:param} summarizes the key parameters. The table \ref{tab:calibr} presents the endogenous variables and their targeted empirical counterparts. Table \ref{tab:mom} compares the data and the model's predictions.
	...
	\begin{table}[h!]
		...
		\begin{tabular}{lccc}
			...
			\tau^{i,e,g}_t & Human capital barriers for other groups & Normalization & 0\\
...
%			\bottomrule
	...
	\begin{table}[h!]
		...
		\begin{tabular}{lccc}
			...
			\tau^{i,w,g}_t & Labor market barriers for other groups & Group gap by occupation\\
