...
\begin{document}
	...
	In contrast to versions of a \cite{Roy:1951}-type model that have been used widely in the previous literature, we allow for the possibility that occupational choice is shaped by comparative as well as absolute advantage.\footnote{Roy's original contribution accounted for selection based on absolute and comparative advantage. Some later work emphasized comparative over of absolute advantage.} The elasticity of income with respect to the worker's human capital can be different from unity and this implies that two workers with identical {\it comparative} but different {\it absolute} advantages (i.e., they can be ranked ``vertically''), may select distinct occupations. This variation gives us additional flexility to account for the evolution of employment shares across occupations separately from changes in the distribution of skills or human capital across jobs.
	
	The question of whether low or high-skill workers are more like to enter (or exit, for that matter) the teaching profession is critical for understanding whether static efficiency gains associated with the elimination of labor market discrimination and educational barriers in non-teaching occupations are muted or amplified by dynamic effects that operate through our model's human capital accumulation channel.\footnote{\cite{Wiswall:2013}, for instance, finds that high-ability math teachers are more likely to exit the teaching profession.} The previous literature has explored the static gains and losses extensively in the context of linear models. The key contribution of our paper is {\it (a)} the introduction of non-linear wage functions that emphasize the role of absolute advantage in addition to the prior focus on comparative advantage and {\it (b)} the intergenerational dynamics of human capital accumulation that mainly operate through the distinct education sector in our model.
	
	In addition to our theoretical contribution, our paper revives and older, but rarely used longitudinal dataset. {\it Project TALENT} surveyed a representative sample of approximately 377 thousand U.S.~high school students in 1959. These same students were interviewed again one, five, and eleven years after they graduated from high school. This dataset was digitized somewhat recently and we use it in combination with the NLSY79 and NLSY97 for our quantitative work. Together, the three surveys paint a rich and detailed picture of American youth spanning more than four decades. Importantly, respondents in all three surveys take aptitude tests that allow us to construct ability profiles, which play a central role in our quantitative work.
	
	Our paper is most closely related, yet complementary, to \cite{Kolm:2024}. They focus on the role that rigid employment contracts play in teaching. Similarly, \cite{Wiswall:2010} explores contract rigidities in the form of licensing requirements, which are a salient feature in the U.S.~context. \cite{Biasi:2021} explores how more flexible contracts benefitted high-quality teachers in Wisconsin. In our model, the wage function captures some degree of contract rigidity and we can quantify the short and long-run implications of increases contract flexibilty. Our paper's main contribution, however, is a sharp characterization of the tradeoff between static and dynamic aggregate welfare gains by way of a micro-founded human capital accumulation technology embedded in an other standard occupational choice model. The importance of teacher quality has already been documented in \cite{Chetty:2014a} and \cite{Chetty:2014b}. Our structural model enables us to identify the forces that shape occupational choice and flexibly characterizes the career choices of prospective teachers with different cognitive abilities over time.
	...
	Without loss of generality, occupation 1 is \textit{teaching}. In this economy, teachers are distinct from other occupations since their technology produces human capital rather than the homogenous final good in the remaining $I-1$ sectors. This kind of distinction is salient since we are assuming that the economy is populated by overlapping generations of workers who accumulate human capital when young. When they reach working age (i.e., they are ``old''), their human capital contributes toward the efficiency units of labor they supply to the labor market, including home production.
	...
	This implies that better teachers work in larger classrooms.
	...
	The resource constraint (\ref{resource}) anchors the class size distribution and equation \eqref{eq:sizedist} characterizes the size distribution of classes.
	...
	\subsection{Values}
	The structure we impose on class size and student-teacher matching simplifies the characterization of an equilibrium considerably. While idiosyncratic heterogeneity still matters, the aggregate state can be summarized by a single moment of the distribution of human capital among current teachers (which we discuss in more detail below) rather than the entire distribution of $\{ h_{T,g} \}_{g = 1}^G$. Thanks to this feature we can characterize stationary equilibria as well as full transition paths sharply.
	
	The idiosyncratic state of a student in this model consists of the $I$-dimensional vector of abilities $\vec{a}$. Recall that we assume $I=2$ for the time being, where $T$ denotes teaching and $O$ lumps together all other occupations. An individual teachers' human capital $h_T$ is not part of the idiosyncratic state space. A single moment of the distribution off all teachers' human capital is part of the aggregate state space.
	...
	Teachers are public sector employees and their compensation is funded by tax revenues. Equation \eqref{eq:bc} is the (local) government's balanced budget requirement, which is a salient feature in the U.S.~context.\footnote{The current version of the model has a single location. Arguably, a more salient environment would feature multiple locations, each of which is subject to a local balanced budget requirement. This extension introduces additional complexities and we focus on this simple version for now.}
	...
	\footnote{In Appendix X, we assume that teachers are compensated for their contribution to the students' human capital and its economics value. One of the key insights of this alternative characterization is that teachers play an economically valuable role and are, seen through the lens of this particular model, under-compensated. The interested reader may want to review the appendix.}
...
%	\newpage
	...
	In Table \ref{tab:samplebywave} we summarize the descriptive statistics by gender, race, parental education, and parental economic status (all in 1960). The racial composition of the sample exhibits some anomalies. The racial classification in the 1960 base year is problematic since almost 60 percent of the sample are in the ``other'', ``unknown'', or ``conflicting'' category.
	%One conceivable explanation for this unusual pattern as well as the {\it rise} of black respondents when the sample size is {\it shrinking} is a retroactive racial re-classification of all the participants in the 5-year post survey.
	\footnote{The classification of respondents into race categories in the base year suffers from inconsistencies (e.g.~self-reported vs.~pollster's classification). Moreover, the sample proportion of black high school students differs from that reported in the 1960 Census.}
	
	Although the high attrition rate from the original sample size of almost 370,000 is a concern, Table \ref{tab:samplebywave} suggests that selection along the respondents' gender and their parents' educational attainment and income is not a major issue. The 11-year post survey appears to be roughly representative of the high school population in 1960. On average, the respondents had slightly more educated parents and higher household income in 1960. This is, arguably, far from ideal but selection is not a major concern at this point.
	...
	%We are mainly interested in the respondents' occupational choice and labor market status eleven years after their presumed high school graduation.
	In the table %\ref{tab:PTscores} 
...
Occupational productivities are recovered from men's occupational shares in 1970, 1990, and 2010. Without loss of generality, we normalize the productivity of home production (occupation 20) to one each year. Figure \ref{fig:prod} illustrates occupational productivities over time. The downward trend in productivities are a reflection of the gradual rise in the share of home production (which includes workers who are not in the labor force) among men.
	...
	We identify the extent of labor market discrimination against women in non-teaching occupations based on differences in the occupational shares of women relative to men year-by-year and we plot them in Figure \ref{fig:bar}. In general, the values of $\tau_w$ for women drift towards zero over time, albeit it with some exceptions. The ``subsidy'' on home production (occupation 20) falls sharply from 1970 to 2010, which reflects the sizeable reduction in the proportion of women engaged in home production over this time.
	...
	The scale parameter of the wage function in teaching, i.e., $\kappa$, determines the fraction of men who select into teaching. The calibrated values are 0.289, 0.190, and 0.169 for 1970, 1990, and 2010, respectively. This downward trend reflects the gradual drop in this share from 2.4 precent in 1970 to 1.9 in 2010.
	
	The parameter that governs the extent of aggregate labor market discrimination against women in non-teaching occupations, $\lambda_f$, is imputed from the fraction of females who select teaching as their occupation. The calibrated steady-state values are 0.879, 1.034, and 0.602 for 1970, 1990, and 2010, respectively.
...
Table \ref{tab:param_time} summarizes the parameters that vary over time. 		
	...
	The evolution of occupational productivities and labor market discrimination -- captured by both $\tau^w$ and $\lambda_f$ -- gives rise to significant changes in the economy's human capital intensity and skill composition of teachers. While the skill composition of female teachers is broadly stable between 1970 and 1990, it shifts to the left between 1990 and 2010 (Figure \ref{fig:fem_ab}). The skill distribution of male teachers, on the other hand, shifts to the right at first and then partially reverts to the left between 1990 and 2010 (Figure \ref{fig:fem_ab}).
...
%	\end{itemize}
	...
	In ongoing work, we are introducing multiple locations to address the stylized fact that there arguably is some sorting between the quality of teachers and the abilities of students across (rather than within) school or school districts. \cite{Biasi:2023} analyses the effect of school finance equalization on student outcomes and intergenerational mobility. In the context of our model, the introduction of multiple locations relaxes the rigidity of teachers' wage contracts: high-income households with, on average, higher-ability children will agglomerate and broaden the tax base the funds teachers' salaries locally and high-ability teachers can move to wealthy districts rather than quit teaching altogether. Positive sorting between teachers and students (households) affects aggregate outcomes through two channels with countervailing effects: on the one hand, competition between districts can attract high-quality workers into teaching, on the other hand, the segregation between low and high-ability students increases inequality. Our framework will be able decompose the effects cleanly.
	%\begin{enumerate}
